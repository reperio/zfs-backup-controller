    ...
    try {
        ...
        if (Config.notification_email.method === 'sendgrid') {
            ...
            const transporterOptions = {
                host: Config.notification_email.smtp_host,
                port: parseInt(Config.notification_email.smtp_port),
                secure: parseInt(Config.notification_email.smtp_port) === 465 ? true : false, // true for 465, false for other ports
                tls: {
                    rejectUnauthorized: Config.notification_email.rejectUnauthorizedTLS
                }
            };

            // add auth to smtp transporter if it was configured
            if (Config.notification_email.smtp_user && Config.notification_email.smtp_password) {
                transporterOptions.auth = {
                    user: Config.notification_email.smtp_user,
                    password: Config.notification_email.smtp_password
                };
            }
                ...
                return new Promise((resolve, reject) => {
                    this.transporter.sendMail(msg, (error, info) => {
                        if (error) {
                            ...
                            app_logger.error(info);
                            reject(error);
                        } else {
                            app_logger.debug(info);
                            app_logger.info(`sent email to ${msg.to}`);
                            resolve(info);
                        }
                    });
                });
